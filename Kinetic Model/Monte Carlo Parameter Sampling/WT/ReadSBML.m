...
MO = sbmlimport('ModelVersoni2_MonteCarlo_WT.xml');
%%
...
%%
% Define intracellular and extracellular reactions
index_r_in = 1;  
index_r_ex = 1;  

for i=1:1:length(MO.Reactions)
    cnr = MO.Reactions(i).Name;
    if  contains(cnr,'exc')|| contains(cnr,'D_')  % reaction for excrecion or dilution are considered external
        suppreactions(index_r_in) = i;
        index_r_in = index_r_in+1;
    else 
        ntwkreactions(index_r_ex) = i;
...
% All kinetics are written with Cytoplasm. or Out. as strucutre. 
